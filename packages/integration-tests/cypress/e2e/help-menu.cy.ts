...
describe("the help menu", () => {
  it("can show help with a keymap", () => {
    ...
    cy.startNeovim({
      startupScriptModifications: ["modify_yazi_config_and_set_help_key.lua", "disable_a_keybinding.lua"],
    }).then(dir => {
      ...
      cy.contains(dir["test-setup.lua"].name)
      ...
      cy.contains(dir["test-setup.lua"].name)
      cy.typeIntoTerminal("q")
      cy.contains(dir["test-setup.lua"].name).should("not.exist")
    })
  })
})
