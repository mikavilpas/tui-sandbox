...
describe('grug-far integration (search and replace)', () => {
  ...
  it('can use grug-far.nvim to search and replace in the directory of the hovered file', () => {
    cy.startNeovim({
      ...
      cy.contains(dir.contents['routes/posts.$postId/should-be-excluded-file.txt'].name)
  ...
  it('can search and replace, limited to selected files only', () => {
    cy.startNeovim({
      ...
      type File = MyIntegrationTestFile
      ...
      // visible in the view
      cy.contains('routes/posts.$postId/should-be-excluded-file.txt' satisfies File).should('not.exist')
    })
  })
})
