import type { MyIntegrationTestFile } from 'server/neovim/environment/testEnvironmentTypes'
import { isFileNotSelectedInYazi, isFileSelectedInYazi } from './utils/yazi-utils'
...
describe('opening files', () => {
  ...
  it('can open files with complex characters in their name', () => {
    cy.startNeovim().then(dir => {
      ...
      // select route.tsx
      cy.typeIntoTerminal(`/${dir.contents['routes/posts.$postId/route.tsx'].name}{enter}`)

      // open the file
      cy.typeIntoTerminal('{enter}')

      // close yazi just to be sure the file preview is not found instead
      cy.get(dir.contents['routes/posts.$postId/adjacent-file.txt'].name).should('not.exist')
  ...
  it('can copy the relative path to the initial file', () => {
    ...
    cy.startNeovim().then(dir => {
      ...
      cy.contains(dir.contents['routes/posts.$postId/route.tsx'].name) // file in the directory
      cy.typeIntoTerminal('{rightArrow}')
      cy.typeIntoTerminal(`/${dir.contents['routes/posts.$postId/adjacent-file.txt'].name}{enter}`)
      ...
      // yazi should now be closed
      cy.contains(dir.contents['routes/posts.$postId/route.tsx'].name).should('not.exist')
      ...
      cy.contains('routes/posts.$postId/adjacent-file.txt' satisfies MyIntegrationTestFile)
    })
  })
  ...
  it('can copy the relative paths of multiple selected files', () => {
    ...
    cy.startNeovim().then(dir => {
      ...
      cy.contains('routes/posts.$postId/adjacent-file.txt' satisfies MyIntegrationTestFile)
      cy.contains('routes/posts.$postId/route.tsx' satisfies MyIntegrationTestFile)
      cy.contains('routes/posts.$postId/adjacent-file.txt' satisfies MyIntegrationTestFile)
    })
  })
  ...
  it('can open multiple files in a directory whose name contains a space character', () => {
    cy.startNeovim({ filename: 'dir with spaces/file1.txt' }).then(dir => {
      ...
      cy.contains('dir with spaces/file1.txt' satisfies MyIntegrationTestFile)
      cy.contains('dir with spaces/file2.txt' satisfies MyIntegrationTestFile)
    })
  })
  ...
  it('can open multiple open files in yazi tabs', () => {
    cy.startNeovim({
      filename: {
        openInVerticalSplits: ['file.txt', 'test-setup.lua', 'dir with spaces/file1.txt'],
      },
      startupScriptModifications: ['modify_yazi_config_and_open_multiple_files.lua'],
    }).then(dir => {
      cy.contains('Hello')
