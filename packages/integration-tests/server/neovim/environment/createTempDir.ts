...
import type { MyTestDirectory } from 'client/neovim-client'
import { constants } from 'fs'
import { access, mkdir, mkdtemp } from 'fs/promises'
import path from 'path'
import { NeovimTestDirectory } from './NeovimTestEnvironment'

export async function createTempDir(): Promise<MyTestDirectory> {
  try {
    const dir = await createUniqueDirectory()

    const directory: MyTestDirectory = {
      rootPathAbsolute: dir,
      rootPathRelativeToTestEnvironmentDir: path.relative(NeovimTestDirectory.testEnvironmentDir, dir),
      contents: {
        'initial-file.txt': {
